			      ...
			      struct pnotify_inode_mark *i_mark)
{
	...
	do {
		...
		// if (unlikely(!idr_preload(GFP_KERNEL))) // XXXKB
		idr_preload(GFP_KERNEL); // XXXKB
			// return -ENOMEM; XXXKB
...
int pnotify_new_watch(struct fsnotify_group *group, u32 pid, u32 arg)
{
	...
	ret = fsnotify_add_mark_locked(&tmp_i_mark->fsn_mark, group, NULL,
				NULL, task, 0);
	if (ret) {
...
SYSCALL_DEFINE4(pnotify_add_watch, int, fd, u32, pid, u32, mask,
		u32, flags)
{
	struct fsnotify_group *group;
	//struct file *filp;
...
SYSCALL_DEFINE2(pnotify_rm_watch, int, fd, u32, pid)
{
	struct fsnotify_group *group;
	struct pnotify_inode_mark *i_mark;
	// struct file *filp;
